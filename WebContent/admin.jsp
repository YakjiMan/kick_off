...
<script>
	...
	{
		var url  = "admin_modify.jsp?no=" + no ;
		var name = "회원정보수정";
	...
	<div class="page">
		<table>
			<tr>
				...
				{
					%>
					<a href="admin.jsp?page=<%= StartBlock - 1 %>">이전</a>
					<%					
				}
				for(int page_no = StartBlock; page_no <= EndBlock; page_no += 1)
				{
					...
					{
						%>
						<a class="page_off" href="admin.jsp?page=<%= page_no %>">
						<%= page_no %>
						</a>&nbsp;
						<%
					}
				...
				{
					%>
					<a href="admin.jsp?page=<%= EndBlock + 1 %>">다음</a>
					<%				
				}
				%>
				</td>

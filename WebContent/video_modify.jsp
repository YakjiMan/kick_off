...
<!-- main 시작 -->
		<div class="main">
			<div class="video_frame">
				<form id="write" enctype="multipart/form-data" method="post" action="video_modifyok.jsp" onsubmit="return DoSave();">
				...
				<%
					...
					{
						%>
						기존 첨부파일 : <a href="down.jsp?no=<%= no %>"><%= vo.getPost_oname() %></a>
						<%
					}
					else
					{
				...
				<div class="clebutton"><a href="video.jsp?mcate=<%= mcate %>&scate=<%= scate %>&sort=<%= sort %>&page=<%= pageno %>" class="button_text">취소</a></div>
				</form>
			</div>
		</div>

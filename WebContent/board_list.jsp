		...
		<tr>
			<td>
				<table class="small_table">
					<tr>
						<td class="board_title"><a href="board_list.jsp">자유게시판</a></td>
					</tr>
				</table>
				<%
					...
					<tr>
						...
						if(login_vo != null)
						{
							if(Integer.parseInt(login_vo.getUser_grade()) >= 1)
							{
								%>
								<td>
									<input type="button" class="write_button" value="글쓰기" onclick="location.href='board_write.jsp?sort=<%= sort %>&page=<%= pageno %>&mcate=<%= mcate %>'">
								</td>
								<%
							}
						}
				...
				<div class="page">
					<table>
						<tr>
							...
							{
								%>
								<a href="board_list.jsp?mcate=<%= mcate %>&sort=<%= sort %>&search_type=<%= search_type %>&keyword=<%= keyword %>&page=<%= StartBlock - 1 %>">이전</a>
								<%					
							}
							...
							{
								%>
								<a href="board_list.jsp?mcate=<%= mcate %>&sort=<%= sort %>&search_type=<%= search_type %>&keyword=<%= keyword %>&page=<%= EndBlock + 1 %>">다음</a>
								<%				
							}
							%>
							</td>
